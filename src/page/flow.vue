  ...
  <div class="flow-container">
    <div class="graph jtk-demo-canvas canvas-wide flowchart-demo jtk-surface jtk-surface-nopan" id="canvas">
      <div v-if="format" id="show_format" style="border:1px  gray solid;line-height:25px;transition:all 0.3s  ease;position:absolute;width:auto;height:auto;padding:15px 15px 15px 15px;background-color:#fff;border-radius:5px;z-index:999">
        <p v-if="!f.isExtend" style="color:goldenrod;border-bottom:1px   rgb(220,220,200) dashed;width:100%;text-align:center" v-for="f in format">{{f.name}}({{ getDataTypeName(f.type_code)}})</p>
        <p v-if="f.isExtend" style="color:dodgerblue;border-bottom:1px   rgb(220,220,200) dashed;width:100%;text-align:center" v-for="f in format"><span v-if="!f.name" style='color:gray'>扩展字段-(待配置)</span><span v-if="f.name" style='color:gray'>{{f.name}}({{ getDataTypeName(f.type_code)}})</span></p>
      </div>
      <div v-on:mouseenter="showExcepetion(index,$event)" v-on:mouseleave="hideExcepetion()" v-on:mousedown.stop.prevent="drag($event,p.id)" v-on:mousemove="move($event,p)" v-on:mouseup="de_drag($event)" v-bind:data-index="index" v-if="p" v-on:contextmenu.stop.prevent="showItemMenu(p,$event,index)" v-on:click="showTip(p,$event,index)" v-bind:class="{'edit':p.isEdit,'window':true,'jtk-node':true,'selected':p.isSelect,'real-node':true,'prepared':p.state.status=='prepared','unprepared':p.state.status=='unprepared'}" v-for="(p,index) in  flowEle.node" v-bind:id="p.id" v-bind:style="{'backgroundSize':'40% 40%',left:p.x?p.x+'px':'90px',top:p.y?p.y+'px':'80px',width:p.width?p.width+'px':'79px',height: compute_height(p)+'px'}">
      ...
      <div v-if="p" v-on:mousedown.stop.prevent="addNode(p.app_id,p.x,p.y,p.source_id)" v-bind:class="{'window':true,'jtk-node':true}" v-for="(p,index) in pre_node" v-bind:id="p.id" v-bind:style="{'backgroundSize':'40% 40%',left:p.x?p.x+'px':'90px',top:p.y?p.y+'px':'90px',width:p.width?p.width+'px':'79px',height: compute_height(p)+'px',opacity:0.3,border:'1px gray dashed'}">
        <div style="width:100%;height:100%;position:absolute">
          <i style="width:100%;height:100%;background-size:50px 50px" v-bind:class="[ getAppByid(p.app_id)?getAppByid(p.app_id).img:'']"></i>
        </div>
      </div>
    ...
    <div class="node" v-on:mouseover="format=null" style="height:100%;">
      <div class="title"><i class="app"></i>应用</div>
      <ul>
        <li v-bind:guid="p.id" v-for="p in flowApp"><i v-bind:class="[p.img]"></i>{{p.name}}</li>
      </ul>
    </div>
    <div class="setting" v-on:mouseover="format=null" style="overflow:hidden">
      <div v-if="show_setting&&setting_node" style="width:100%;height:90%;overflow:auto">
        <div class="setting_module" v-for="p in setting">
          <div v-on:click="flodSettingModule(p,$event)" style="width:100%;height:50px; line-height:50px;padding-left:15px"><i style="height:100%" v-bind:class="{open:p.isOpen,close:!p.isOpen}"></i><span style="padding-left:20px">{{p.name}}</span></div>
          <parameters v-bind:floor="0" v-bind:style="{display:p.isOpen?'block':'none'}" v-on:getFormatFromEndpoint="getFormatFromEndpoint" v-on:addEndpoint="addEndpoint" v-on:deleteEndpoint="deleteEndpoint" v-on:updateEndpoint="updateEndpoint" v-if="p.parameters&&p.parameters.length>0" v-bind:parameters="p.parameters" v-bind:node="setting_node" v-bind:endpoints="setting_endpoints(setting_node.id)" />
        </div>
...
<script>
  ...
  import '../assets/scss/flow_mana.scss'
  ...
  import Vue from 'vue'
  import drowlist from '../components/common/drowlist'
    ...
    methods: {
      ...
      flodSettingModule: function (p, e) {
        ...
        if (p.isOpen) {
          ...
          $(e.currentTarget).next().slideUp(0);
        }
      },
      ...
      computedDefaultPosition: function (node, x, y, isInitAdd) {
        ...
        var Xoffset = 120;
        var Yoffset = 120;
        var test_node = { x: x || 80, y: y || 80, width: node.width || 80, height: node.height || 80 };
